...
| `0106-SMOUK-0.0.104-transcription-review-dialog.patch` | **scripts / openshot-qt** | Adds a ParlaBE sentence-review dialog before subtitle Timeline placement, preserving original timecodes and allowing manual edits. |
...
| `0122-SMOUK-0.0.120-enforce-safe-post-review-width.patch` | **openshot-qt** | Uses the effective 40-character post-review limit so no corrected subtitle can exceed the 9:16 safe width. |
...
| `0127-SMOUK-0.0.125-mask-track-output-and-settings.patch` | **openshot-qt** | Uses reversible Track.enabled output switching, persists MASK settings through declared OpenShot preferences, fixes timed X/Y keyframes, and names the general dialog SETTINGS VERTICALIZATION. |
| `0128-SMOUK-0.0.126-visible-verticalization-settings-titlebar.patch` | **openshot-qt** | Places the general settings gear in OpenShot's actual dock title-bar implementation so it cannot be replaced by later dock styling. |
| `0129-SMOUK-0.0.127-mask-eye-live-output.patch` | **openshot-qt** | Applies the MASK eye to a reversible full-duration alpha curve so preview and render update immediately despite OpenShot ignoring layer-only update diffs. |
| `0130-SMOUK-0.0.128-show-mask-track-button.patch` | **openshot-qt** | Adds an explicit SHOW MASK TRACK action that reconstructs the saved PSD track only when missing, keeping track creation separate from the eye output switch. |
| `0131-SMOUK-0.0.129-configurable-subtitle-settings.patch` | **openshot-qt** | Adds persistent subtitle typography, colors, border, position, line spacing, alignment, and safe-width-aware two-line layout controls to the general Verticalization settings dialog. |
